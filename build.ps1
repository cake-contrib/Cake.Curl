...
Invoke-Expression "& `"$CAKE_EXE`" `"$Script`" -experimental -target=`"$Target`" -configuration=`"$Configuration`" -packageOutputDirectory=`"$PackageOutputDirectory`" -verbosity=`"$Verbosity`""
exit $LASTEXITCODE
